...
game =
  Game
    { prepare = do
        ...
        g <- liftIO $ loadMapFile "group.json"
        (m, objs, sps) <- liftIO $ parseMapFile offset unit g

        maps <- liftIO $ newIORef (Maps unit m offset sps)
        ...
        let update c g o = GameCtx
              $ case c of
                GameReset -> do
                  E.catch
                    ( do
                        f <- liftIO $ loadMapFile "group.json"
                        let unit = 20
                        (m, objs, sps) <- liftIO $ parseMapFile offset unit f

                        writeIORef maps (Maps unit m offset sps)
                ...
                GameLoad i -> do
                  E.catch
                    ( do
                        ...
                        (m, objs, sps) <- liftIO $ parseMapFile offset 5 f

                        writeIORef maps (Maps 5 m offset sps)
                        (unis, s) <- buildRenderer win (player `V.cons` objs)

                        gs <- gridShader win unis f 5 offset
                        let r = renderWith unis s $ \vpSize -> do
                              clear
                              gs vpSize
                        liftIO $ writeIORef renderings r
...
updateFrame env update renderText camera target input = do
  ...
  let inp = "target: " ++ show target ++ "\ncamera: " ++ showV3F camera ++ "\nfps: " ++ showF fps ++ "\nmid:" ++ show input.n ++ "\np: " ++ show pos
      visibleBox :: BB.Box V2 Int = BB.inflate 0 (floor <$> target.bbox `BB.move` (fromIntegral <$> pos))
  _ <-
    update
      ( if
