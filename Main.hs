...
import qualified Graphics.GPipe.Context.GLFW.Input
                                               as Input
 ...
 where
  ...
  renderMesh bs ts ms Mesh {..} = do
    ...
    writeBuffer
      vbuf
      0
      [ (dvPosition + offset, dvNormal, dvUv) | DrawVertex {..} <- vertices ]
  ...
  newPrimitiveArray
    :: forall b i a
     . (BufferFormat b, Integral i, IndexFormat b ~ i)
    => Buffer os a
    -> Maybe (Buffer os b)
    -> Render os (PrimitiveArray Triangles a)
...
main :: IO ()
main =
  runContextT (GLFW.defaultHandleConfig { GLFW.configEventPolicy = Nothing })
    $ do
        win <- newWindow (WindowFormatColorDepth RGBA8 Depth16)
...
readInput win keyInputSink = do
  ...
  return
    $ if closeRequested == Just True || closeKeyPressed then Nothing else t'
 where
